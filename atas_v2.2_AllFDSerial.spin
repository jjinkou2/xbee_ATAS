...
' String = "03,CR,04,CR,05,CR,[RSSI],CR,[SSID],CR,[loop]"
...
PUB XB_to_Str(desString,len) | i

    bytefill (desString,0,len)
    XB.rxFlush                    ' Empty buffer for data from XB

    i:=0
{    repeat  while (XBchar:=XB.rxCheck)==-1    'wait for data
    byte[desString][0]:=XBchar
    i:=1}

    repeat while (XBchar:=XB.RX)=>0 and i<len
    'repeat  while (XBchar:=XB.rx)<>-1 ' and i<len
